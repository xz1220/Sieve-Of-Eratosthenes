...
int main(int argc, char *argv[])
{
   ...
   if ((2 + proc0_size) < sqrt_n) {
    ...
    printf("数据计算完成： id:%d, n:%ld, p:%d, low_value:%ld, high_value:%ld, size:%ld \n",id,n,p,low_value,high_value,size);
   // elapsed_time2= -MPI_Wtime();
   ...
   // if (!id) Time(elapsed_time2);
    printf("id:%d, 初始化数组成功,进入循环",id);
   // elapsed_time3= -MPI_Wtime();
   long cacheline = (1<<12)*8;
   // int cacheline = 1<<15;
   long number,multi_cacheline=0;
   long high_value_temp;
